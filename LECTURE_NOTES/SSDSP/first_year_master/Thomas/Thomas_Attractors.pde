float b = 0.19;
float dt = 0.005;  // Passo di tempo 

int att_size = 1500;

PVector[] thomas = new PVector[att_size];
PVector[] james = new PVector[att_size];
...
void draw() {
  ...
  strokeWeight(0.01);
  for (int i = 0; i < att_size; ++i) {
    vertex(thomas[i].x, thomas[i].y, thomas[i].z);
  }
  endShape();
